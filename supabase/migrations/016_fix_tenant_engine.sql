-- ERP General — 016 Fix tenant_id propagation in engine functions (WP-D1 follow-up)
-- fn_notify and fn_after_status_change were created before 015 added tenant_id
-- to handoff_records and notifications.  Update them to carry p_doc.tenant_id.

-- ============================================================
-- fn_notify — add tenant_id parameter (DEFAULT NULL → fallback to fn_current_tenant)
-- ============================================================
CREATE OR REPLACE FUNCTION fn_notify(p_user uuid, p_title text, p_body text, p_doc uuid,
                                     p_tenant_id uuid DEFAULT NULL) RETURNS void
LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  INSERT INTO notifications (user_id, title, body, document_id, tenant_id)
  VALUES (p_user, p_title, p_body, p_doc,
          coalesce(p_tenant_id, fn_current_tenant(),
                   '00000000-0000-0000-0000-000000000001'::uuid))
$$;

-- ============================================================
-- fn_after_status_change — pass p_doc.tenant_id to handoff_records and fn_notify
-- ============================================================
...
BEGIN
  ...
  FOR m IN SELECT * FROM handoff_map WHERE doc_type = p_doc.doc_type AND trigger_status = p_doc.status AND status = 'ACTIVE' LOOP
    ...
    VALUES (m.id, p_doc.id, p_actor, v_dept, m.to_role, fn_now() + make_interval(hours => m.sla_hours),
            coalesce(p_doc.tenant_id, '00000000-0000-0000-0000-000000000001'::uuid));
    FOR u IN
    ...
    LOOP
      PERFORM fn_notify(u.id, m.expected_action || ' — ' || p_doc.number,
                        v_type_name || coalesce(': ' || p_doc.title, ''), p_doc.id, p_doc.tenant_id);
    END LOOP;
  END LOOP;
  IF p_doc.status IN ('APPROVED','REJECTED','POSTED','PAID','ON_HOLD','RECEIVED','CLOSED','ONBOARDED')
    ...
    PERFORM fn_notify(p_doc.created_by, format('%s %s → %s', v_type_name, p_doc.number, p_doc.status),
                      coalesce(p_doc.title, ''), p_doc.id, p_doc.tenant_id);
  END IF;
END $$;
