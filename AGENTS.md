# AGENTS.md — ERP-General Universal ERP Project

> **Mục đích**: File hướng dẫn toàn diện để Codex đọc, hiểu và thực thi xây dựng hệ thống ERP phổ quát.
...
16. [Checklist tổng hợp cho Codex](#16-checklist-tổng-hợp)
...
|-------|-----|----------|-------------|
| L1 | **Core** | Foundation — AGENTS.md, app-map, routing | File này + docs/app-map/ |
...
```
AGENTS.md (root)
├── packages/api/AGENTS.md        → Backend context
├── packages/web/AGENTS.md        → Frontend context  
├── packages/shared/AGENTS.md     → Shared types/utils
└── docs/AGENTS.md                → Documentation context
```
- Mỗi package/module có AGENTS.md riêng kế thừa từ root
- Codex đọc từ root → deep vào module đang làm việc
...
routing_rules:
  - pattern: "packages/api/**"
    context: ["packages/api/AGENTS.md", "docs/app-map/001-api-overview.md"]
  - pattern: "packages/web/**"
    context: ["packages/web/AGENTS.md", "docs/app-map/002-web-overview.md"]
  - pattern: "**/*.test.*"
...
- Lessons learned → cập nhật AGENTS.md
...
#### P15: AGENTS.md as Single Source of Truth
- Mọi quyết định kiến trúc ghi trong AGENTS.md hoặc ADR
- Conflict resolution: AGENTS.md wins over comments in code
...
```
ERP-general/
├── AGENTS.md                          ← FILE NÀY (root context)
├── .Codex/
...
Deliverables:
  □ AGENTS.md (file này) — project context
...
## 16. CHECKLIST TỔNG HỢP CHO Codex

### Khi bắt đầu một feature mới:
```
□ 1. Đọc AGENTS.md (file này) + app-map doc liên quan
...
> **Codex**: Khi đọc file này, hãy tuân thủ mọi quy tắc, pattern, và checklist ở trên.
